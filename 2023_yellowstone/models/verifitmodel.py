...
class VerifitModel:
    ...
    def _get_measured_spls(self, root, filename):
        ...
        spls.insert(loc=0, column='freq', value=self.freqs_12oct)
        
        # Grab desired frequencies, if specified
        if self.desired_freqs:    
            spls = spls[spls['freq'].isin(self.desired_freqs)]
        ...
        for key, value in test_key.items():
            for side in sides:
                try:
                    ...
                    pass
                    #print(f"No data for {key, '->', value} in {filename}")

        #####################
        # Create Data Frame #
        #####################
        targets = pd.DataFrame(target_dict)
        targets.insert(loc=0, column='freq', value=self.freqs_audio)
    ...
    def get_data(self):
        """ Pull measured SPLs and target SPLs from Verifit .xml file.
            ...
            Use LEVELS to find all possible curves.
        """
        # Display to console
        ...
        for file in self.files: 
            print(f"verifitmodel: Processing {file}")
        ...
        self.measured = self.measured.apply(pd.to_numeric, errors='ignore')
        self.targets = pd.concat(target_dfs, ignore_index=True)
        self.targets = self.targets.apply(pd.to_numeric, errors='ignore')
