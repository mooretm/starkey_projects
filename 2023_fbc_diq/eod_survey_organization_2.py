""" Organize FBC DiQ raw End of Day survey data for
    app version number 1.1.2.
...
eod_files = [file for file in ALL_FILES if os.path.basename(file).split('-')[0] == "EOD"]

"""End of Day Survey Organization."""
...
for file in eod_files:
    ...
    # Update header names
    temp.columns = ['heard_chirp', 'rating', 'more_info', 'comments', 'memory']
...
# keys = sorted(list(data['noise'].unique()))
# values = [3,2,1]
# noise_dict = {k:v for k,v in zip(keys, values)}
# print(noise_dict)
# for k, v in noise_dict.items():
#     mask = data['noise'] == k
#     data['noise'][mask] = noise_dict[k]
...
# Update "Personal" and "Personal2" to "on" and "off", respectively
mask = data['memory'] == "Personal"
data['memory'][mask] = "on"
data['memory'][-mask] = "off"

print(data)

data.to_csv(f'{SUBJECT}_eod_survey.csv', index=False)
